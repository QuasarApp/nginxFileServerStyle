...
$(document).ready(function () {
  ...
  // Add back button.
  $("h1").html(
    '<p><img alt="логотип" style="margin-top: 7px; margin-left: 0px; padding: -1px; border-radius: 5px; border: 1px solid #aaa;" src="/.html/logo2.png"></p>'
  );
  ...
  // Add search box.
  $("h1").prepend(
    '<form id="custom-search-form" class="form-inline pull-right"><div class="btn-group"><input id="searchBox" placeholder="Найти на странице" type="search" class="form-control"> <span id="searchclear" class="glyphicon glyphicon-remove-circle"></span></div></form>'
  );
  ...
  // Add titles.
  $("pre").prepend(
    '<div class="header">Name                                                   Time                 Size</div>'
  );
  ...
  $("pre a").each(function () {
    ...
    for (var i = 0; i < formats.length; i++) {
      if (fileExt.toLowerCase() == formats[i].toLowerCase()) {
        ...
        $(this).html(
          '<img class="icons" src="/.html/icons/' +
            formats[i] +
            '.png"></img></a>' +
            oldText
        );
    ...
    if (
      ...
      var oldText = $(this).text();
      $(this).html(
        '<img class="icons" src="/.html/icons/folder.png">' +
    ...
    if (found == 0) {
      var oldText = $(this).text();
      $(this).html(
        '<img class="icons" src="/.html/icons/error.png">' + oldText
      );
      return;
    }
  });
